    ...
    try {
      if (confirmModal.type === 'delete_memorial' && confirmModal.itemId) {
        ...
        if (success) {
          ...
          alert(`Erro ao apagar: ${(error as any)?.message || "Tente novamente."}`);
        }
      }
        ...
        if (success) {
          ...
          alert(`Erro ao excluir conta: ${(error as any)?.message || "Tente novamente."}`);
        }
      }
    } catch (err) {
      ...
      <div className="bg-white border-b border-slate-200 shadow-sm relative z-10">
        ...
        <div className="relative h-48 md:h-64 bg-slate-200 group">
          {userCover ? (
            ...
            <div className="w-full h-full bg-gradient-to-r from-brand-600 to-rose-400 flex items-center justify-center">
